...
def get_groups_id_by_direction_id(direction_id):
    with Session(engine) as session:
        res = session.query(GroupsConnectionToDirection).filter(GroupsConnectionToDirection.direction_id.is_(direction_id)).all()
        result = [el.group_id for el in res]
        return result

def get_direction_id_by_group_id(group_id):
    with Session(engine) as session:
        connection = session.query(GroupsConnectionToDirection).filter(GroupsConnectionToDirection.group_id.is_(group_id)).one_or_none()
...
def delete_groups_by_direction_id(direction_id):
    with Session(engine) as session:
        ...
        session.query(GroupsConnectionToDirection).filter(GroupsConnectionToDirection.group_id.in_(groups_ids), GroupsConnectionToDirection.direction_id.is_(direction_id)).delete(synchronize_session=False)
        session.commit()
        return True