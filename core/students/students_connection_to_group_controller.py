...
def get_students_id_by_group_id(group_id):
    with Session(engine) as session:
        students = session.query(StudentsConnectionToGroup).filter(StudentsConnectionToGroup.group_id == group_id, StudentsConnectionToGroup.date_of_deduction.is_(None)).all()
...
def update_student_to_group_connection(data):
    ...
    with Session(engine) as session:
        connection = session.query(StudentsConnectionToGroup).filter(StudentsConnectionToGroup.group_id == group_id, StudentsConnectionToGroup.student_id == student_id, StudentsConnectionToGroup.date_of_deduction.is_(None)).first()
...
def get_groups_by_student_id(student_id):
    with Session(engine) as session:
        connections = session.query(StudentsConnectionToGroup).filter(StudentsConnectionToGroup.student_id == student_id).all()
        result = []
        for connection in connections:
            teacher_id = session.query(TeachersConnectionToGroup).filter(TeachersConnectionToGroup.group_id == connection.group_id).first()
            teacher = session.query(Teacher).filter(Teacher.id == teacher_id.teacher_id).first()
